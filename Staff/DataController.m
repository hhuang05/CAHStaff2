...
// name with, e.g. "F" + "Maj"
-(void)addKey:(NSString*)theKey toChords:(NSArray*)theChords{
    for(Chord *c in theChords){
        [c setupKey:theKey];
    }
}
...
-(void)keySignatureWasChosen:(NSString*)choice
{
    ...
    [self addKey:choice toChords:_currentChords];
    
    if(keySignaturetoDraw){
        AppDelegate *mainDelegate = (AppDelegate*)[[UIApplication sharedApplication]delegate];
        [mainDelegate.viewController.staffController changeScale:keySignaturetoDraw];
        [mainDelegate.viewController.chordController setUpChords:(NSArray*)_currentChords];
    }
    else
